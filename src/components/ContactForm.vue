...
const handleServerResponse = async (
	...
	if (ok) {
		...
		textPlaceholder.value = `${$t("ContactForm.errorMsg")}: "${
			status.info.msg
		}"`;
		mailData.mailMessage = "";
	}
...
const textPlaceholder = ref(`${$t("ContactForm.enterMsg")}`);
...
function sendMail() {
	...
	axios({
		...
		.then(() => {
			handleServerResponse(true, $t("ContactForm.msgSent"));
		})
		.catch((error) => {
...
<template>
	<div class="h-full w-full contact">
		<label for="to" class="pl-4" v-text="$t('ContactForm.sender')"></label>
		<input
			class="pl-2"
			type="email"
			name="to"
